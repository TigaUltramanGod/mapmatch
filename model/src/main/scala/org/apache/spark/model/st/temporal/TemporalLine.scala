...
import org.apache.spark.model.st.temporal.util.DateTimeUtils
...
trait TemporalLine[T <: TemporalEntity] {
  ...
  /**
    ...
    **/
  def getTimeSpanInSec: Long = getEndTime.toInstant.getEpochSecond - getStartTime.toInstant.getEpochSecond

  /**
    * time span of the time line(hour)
    **/
  def getTimeSpanInHour: Double = getTimeSpanInSec.asInstanceOf[Double] / DateTimeUtils.SECONDS_PER_HOUR
}
...
object TemporalLine {
  ...
  def sortByTime[T <: TemporalEntity](timeEntities: Array[T]): Array[T] = {
    timeEntities.sorted(new Ordering[TemporalEntity] {
      override def compare(x: TemporalEntity, y: TemporalEntity): Int = {
        x.compareTime(y)
      }
    })
  }
